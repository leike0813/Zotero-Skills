...
export class SkillRunnerClient {
  ...
  private findStep(request: HttpStepsRequest, stepId: string) {
    return request.steps.find((step) => step.id === stepId);
  }

  private async executeCreateStep(step: HttpStepDefinition) {
  ...
  private async executeUploadStep(
    ...
    form.append("file", new Blob([zipBytes]), "inputs.zip");
  ...
  private async executeBundleStep(step: HttpStepDefinition, requestId: string) {
    const response = await this.fetchImpl(
      this.buildUrl(interpolatePath(step.request.path, { request_id: requestId })),
      {
        method: step.request.method,
  ...
  async executeHttpSteps(request: HttpStepsRequest): Promise<SkillRunnerExecutionResult> {
    ...
    if (!createStep || !uploadStep || !pollStep || !bundleStep) {
      throw new Error("http.steps request missing create/upload/poll/bundle step");
    }
