...
The editor UI MUST expose references payload entries as editable form rows and support structural operations.
...
For multiple valid inputs in a single trigger, editor windows MUST be shown one-by-one and identify target parent context.

#### Scenario: Multiple valid references notes in one trigger
- **WHEN** workflow trigger resolves more than one valid input unit
- **THEN** editor windows SHALL open sequentially (not in parallel)
- **AND** each window SHALL clearly indicate which parent item/note context is being edited
